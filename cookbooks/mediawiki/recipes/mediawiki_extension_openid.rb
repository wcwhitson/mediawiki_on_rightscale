...
if node[:mediawiki_extensions][:openid][:enable] == "true"
...
elsif node[:mediawiki_extensions][:openid][:enable] == "false"
  file "#{node[:mediawiki][:installation_directory]}/#{node[:mediawiki][:ext_config_dir]}/OpenIDSettings.php" do
    :delete
  end
else
  log "ERROR: No possible action for OpenID extension."
end
